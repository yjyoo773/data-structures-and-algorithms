...
in each house (fewest to most). If a house has the same number of children, sort alphabetically by 
house name.
...
//     let ppl = Object.values(arr[i]);
//     return ppl[0] === character && ppl[2].length > 0 ? true : false;
//   }
// };
...
const hasChildrenValues = (arr, character) => {
  for (let i = 0; i < arr.length; i++) {
    let ppl = Object.values(arr[i]);
    if (ppl[0] === character && ppl[2].length > 0) return true;
  }
  return false;
}
/* ------------------------------------------------------------------------------------------------
...
const hasChildrenEntries = (arr, character) => {
  let char = arr.find((x) => x.name === character);
  return char.children.length > 0 ? 1 : -1;
};
