...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i <= strToArr.length; i++) {
    result.push(strToArr.slice(i).join(""));
  }
  return result;
};
...
Write a function name wordsToCharList that, given a string as input, returns a new array where every
 element is a character of the input string.
...
You are making a grocery list for ingredients needed in the gruffalo crumble recipe, below. Rather than taking 
the entire recipe, you only want a list of the item names.

Write a function named listFoods that takes in the recipe and returns a new array of the food items without any 
amount or units. Just the name. For example, '1 cup flour' will return 'flour'.
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < ingredients.length; i++) {
    let firstSpace = ingredients[i].indexOf(" ")+1;
    let newIng = ingredients[i].slice(firstSpace)
    let idx = newIng.indexOf(' ')+1;
    result.push(newIng.slice(idx))
  }
  return result;
};
...
Write a function named removeLastCharacters that takes in a string and a number. The numberOfCharacters 
argument determines how many characters will be removed from the end of the string. Return the 
resulting string.

If the numberOfCharacters argument is greater than the length of the input string, the function should 
return an empty string.

If the numberOfCharacters argument input is a negative number, the function should return the input 
